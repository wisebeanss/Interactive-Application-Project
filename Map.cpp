...
void Map::printSidebar(int carriageNum, int carriageRoom, bool uiActive, Player& player, const std::string uiBuffer[13]) const {
	...
	for (int i = 0; i < 13; i++) {
		...
		switch (i) {
		case 0:lineBuffer += formatPanelLine("          [ CONTROLS ]", targetWidth); break;
		case 1:lineBuffer += formatPanelLine("   WASD : Move Player", targetWidth); break;
