...
bool Puzzle::ROOMS(int roomID, Map& map)
{
    if (roomID == 1)
    {
        ...
        if (getCurrentRoom() != 1)
        {
            ...
            map.setObjects(new Clocks("Clock4: A clock was 5 minutes slow.\n", 4, 8, 103));
    ...
    if (roomID == 2)
    {
        ...
        {
            map.clearObjects();
            map.setObjects(new PhotoFragment("Photo Fragment 1:\nA 'Hand'\n", 1, 4, 101));
            map.setObjects(new PhotoFragment("Photo Fragment 2:\nA \"Face\"\n", 4, 4, 102));
            map.setObjects(new PhotoFragment("Photo Fragment 3:\n\"The Other person\" \n", 1, 8, 103));
    ...
    if (roomID == 5)
    {
        if (getCurrentRoom() != 5)
        {
         ...
         /*   map.setObjects(new Lockers("The locker is empty.A faded sticker reads: "PROPERTY OF STAFF" ", 23, 6, 301));
            ...
            map.setObjects(new Note(" Another ticket says: I sat opposite you. Your seat is 17. ", 23, 6, 301));
...
void Puzzle::setCurrentRoom(int room) {
    currentRoom = room; 
}

int Puzzle::getCurrentRoom() {
    return currentRoom; 
}
